...
func concatServerToolResult(chunks []*ServerToolResult) (*ServerToolResult, error) {
	if len(chunks) == 0 {
		return nil, nil
	}
	if len(chunks) == 1 {
	...
	var (
		...
		textEditorCodeExecutionResult *TextEditorCodeExecutionResult
		toolSearchToolBm25Result      *ToolSearchToolResult
		toolSearchToolRegexResult     *ToolSearchToolResult
		seenType                      reflect.Type
	)
	for _, chunk := range chunks {
		...
		if err := setServerToolResult(&seenType, chunk.WebSearch, &webSearchResult); err != nil {
			return nil, err
		}
		if err := setServerToolResult(&seenType, chunk.WebFetch, &webFetchResult); err != nil {
			return nil, err
		}
		if err := setServerToolResult(&seenType, chunk.CodeExecution, &codeExecutionResult); err != nil {
			return nil, err
		}
		if err := setServerToolResult(&seenType, chunk.BashCodeExecution, &bashCodeExecutionResult); err != nil {
			return nil, err
		}
		if err := setServerToolResult(&seenType, chunk.TextEditorCodeExecution, &textEditorCodeExecutionResult); err != nil {
			return nil, err
		}
		if err := setServerToolResult(&seenType, chunk.ToolSearchToolBm25, &toolSearchToolBm25Result); err != nil {
			return nil, err
		}
		if err := setServerToolResult(&seenType, chunk.ToolSearchToolRegex, &toolSearchToolRegexResult); err != nil {
			return nil, err
		}
	}

	switch {
	case webSearchResult != nil:
		return &ServerToolResult{WebSearch: webSearchResult}, nil
	case webFetchResult != nil:
		return &ServerToolResult{WebFetch: webFetchResult}, nil
	case codeExecutionResult != nil:
		return &ServerToolResult{CodeExecution: codeExecutionResult}, nil
	case bashCodeExecutionResult != nil:
		return &ServerToolResult{BashCodeExecution: bashCodeExecutionResult}, nil
	case textEditorCodeExecutionResult != nil:
		return &ServerToolResult{TextEditorCodeExecution: textEditorCodeExecutionResult}, nil
	case toolSearchToolBm25Result != nil:
		return &ServerToolResult{ToolSearchToolBm25: toolSearchToolBm25Result}, nil
	case toolSearchToolRegexResult != nil:
		return &ServerToolResult{ToolSearchToolRegex: toolSearchToolRegexResult}, nil
	default:
		return nil, fmt.Errorf("no valid server tool result to concat")
	}
}

func setServerToolResult[T any](seenType *reflect.Type, chunk *T, result **T) error {
	if chunk == nil {
		return nil
	}

	chunkType := reflect.TypeFor[*T]()
	if *seenType != nil && *seenType != chunkType {
		return fmt.Errorf("server tool result type mismatch: expected '%v', but got '%v'", *seenType, chunkType)
	}
