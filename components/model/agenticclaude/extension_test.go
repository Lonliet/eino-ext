...
func TestConcatServerToolResult(t *testing.T) {
	t.Run("same type keeps concrete result", func(t *testing.T) {
		got, err := concatServerToolResult([]*ServerToolResult{
			nil,
			{
			...
			{
				CodeExecution: &CodeExecutionResult{
					Type: CodeExecutionResultTypeError,
					Error: &CodeExecutionResultError{
						Code: "tool_error",
					},
				},
			},
		...
		if err != nil {
			t.Fatalf("concatServerToolResult() error = %v", err)
		}
		if got == nil || got.CodeExecution == nil {
			t.Fatalf("concatServerToolResult() = %#v", got)
		}
		if got.CodeExecution.Type != CodeExecutionResultTypeError || got.CodeExecution.Error == nil || got.CodeExecution.Error.Code != "tool_error" {
			t.Fatalf("code execution result = %#v", got.CodeExecution)
		}
	})

	t.Run("type mismatch", func(t *testing.T) {
		_, err := concatServerToolResult([]*ServerToolResult{
			{WebSearch: &WebSearchResult{Type: WebSearchResultTypeResult}},
			{WebFetch: &WebFetchResult{Type: WebFetchResultTypeResult}},
		})
		if err == nil || !strings.Contains(err.Error(), "server tool result type mismatch") {
			t.Fatalf("concatServerToolResult() error = %v", err)
		}
	})

	t.Run("no valid result", func(t *testing.T) {
		got, err := concatServerToolResult([]*ServerToolResult{nil, nil})
		if err == nil || err.Error() != "no valid server tool result to concat" {
			t.Fatalf("concatServerToolResult() = %#v, error = %v", got, err)
		}
	})
}
