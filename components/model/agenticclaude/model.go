...
func (m *Model) genRequestAndOptions(input []*schema.AgenticMessage, options *model.Options,
	...
	reqOpts = appendServerToolBetaHeaders(reqOpts, specOptions.serverTools, specOptions.customHeaders)
...
func appendServerToolBetaHeaders(reqOpts []option.RequestOption, serverTools []*ServerToolConfig, customHeaders map[string]string) []option.RequestOption {
	customBetas := splitHeaderValues(customHeaders[headerAnthropicBeta])
	betas := collectServerToolBetaHeaders(serverTools)
	for _, beta := range betas {
		if _, ok := customBetas[beta]; ok {
			continue
		}
